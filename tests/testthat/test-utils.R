...
for (nCores in seq_len(2)) {
    ...
    test_that(paste("getG.symDMatrix", "on", nCores, "cores"), {
        ...
        for (nTasks in c(1, 3)) {
            G2 <- getG.symDMatrix(X = W, folder = paste0(testPath, "test-", randomString()), nBuffers = 3, nTasks = nTasks, nCores = nCores, verbose = FALSE)
            expect_equal(G2[], G1)
        }
    ...
    test_that(paste("getGi", "on", nCores, "cores"), {
        ...
        for (nBuffers in c(1, 3)) {
            for (nTasks in c(1, 3)) {
                ...
                G <- G / mean(diag(G))
                G2 <- getG(x = X, scaleCol = TRUE, scaleG = TRUE, nBuffers = nBuffers, nTasks = nTasks, nCores = nCores, verbose = FALSE)
                expect_equal(G, G2, check.attributes = FALSE)

                # without scaling to average diagonal = 1 (scaleG)
                G <- tcrossprod(scale(X))
                G2 <- getG(x = X, scaleCol = TRUE, scaleG = FALSE, nBuffers = nBuffers, nTasks = nTasks, nCores = nCores, verbose = FALSE)
                expect_equal(G, G2, check.attributes = FALSE)

                # without scaling columns, but scaling average diagonal = 1 (scaleG)
                G <- tcrossprod(scale(X, center = TRUE, scale = FALSE))
                G <- G / mean(diag(G))
                G2 <- getG(x = X, scaleCol = FALSE, scaleG = TRUE, nBuffers = nBuffers, nTasks = nTasks, nCores = nCores, verbose = FALSE)
                expect_equal(G, G2, check.attributes = FALSE)

                # no scaling at all
                G <- tcrossprod(scale(X, center = TRUE, scale = FALSE))
                G2 <- getG(x = X, scaleCol = FALSE, scaleG = FALSE, nBuffers = nBuffers, nTasks = nTasks, nCores = nCores, verbose = FALSE)
                expect_equal(G, G2, check.attributes = FALSE)

                # neither scaling nor centering
                G <- tcrossprod(X)
                G2 <- getG(x = X, centerCol = FALSE, scaleCol = FALSE, scaleG = FALSE, nBuffers = nBuffers, nTasks = nTasks, nCores = nCores, verbose = FALSE)
                expect_equal(G, G2, check.attributes = FALSE)
    ...
    test_that(paste("summarize", "on", nCores, "cores"), {
        ...
        for (bufferSize in c(3, 6)) {
            for (nTasks in c(1, 3)) {
                ...
                expect_equal(summarize(genotypes, i = c(1, 3), bufferSize = bufferSize, nTasks = nTasks, nCores = nCores), computeDummy(i = c(1, 3)))
                expect_equal(summarize(genotypes, j = c(1, 3, 5), bufferSize = bufferSize, nTasks = nTasks, nCores = nCores), computeDummy(j =  c(1, 3, 5)))
                expect_equal(summarize(genotypes, i = c(1, 3), j = c(1, 3, 5), bufferSize = bufferSize, nTasks = nTasks, nCores = nCores), computeDummy(i = c(1, 3), j = c(1, 3, 5)))
